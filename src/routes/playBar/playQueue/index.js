...
class PlayQueue extends React.Component {
	...
	render() {
		let { songList=[], currentSong } = this.props;
		...
		const isShow = true;
		const { currentTime,isPlayqueueShow, handleDelClick } = this.props;
						...
						<div className ="playqueue-playlist-content" >
							{
								songList.length !==0 && songList.map(( item,index )=>{
									return (
										<div className = {item.id === (currentSong.id) ? "playqueue-playlist-list active" : "playqueue-playlist-list"} key = { index }>
											<div>
												<span className = {"playqueue-playlist-playing"}></span>
												<span className = "playqueue-playlist-songname">歌曲id: {item.id}</span>
											</div>
											<div className = "playqueue-playlist-list-right">
												<div className = "playqueue-playlist-icons">
													...
													<i className = "playqueue-playlist-icon4" onClick={handleDelClick}></i>
												</div>
												{/* <span className = "playqueue-playlist-singer"></span> */}
												<span className = "playqueue-playlist-sonngtime">4:07</span>
...
const mapDispatch = (dispatch) => ({
	handleDelClick() {
		dispatch({
			type: "player/del-song"
		});
	},
});

export default connect(({player}) => player, mapDispatch)(PlayQueue); 
